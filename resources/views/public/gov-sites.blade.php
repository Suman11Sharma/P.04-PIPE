@php
    ...
    $provinces = Constituency::distinct()->pluck('province_name')->sort()->values();

    $provinceData = [
        'Eastern Cape' => ['abbr' => 'EC', 'capital' => 'Bhisho', 'website' => 'https://www.ecprov.gov.za', 'youtube' => 'https://youtube.com/@EasternCapeProvince'],
        'Gauteng' => ['abbr' => 'GP', 'capital' => 'Johannesburg', 'website' => 'https://www.gauteng.gov.za', 'youtube' => 'https://youtube.com/@GautengProvince'],
        'KwaZulu-Natal' => ['abbr' => 'KZN', 'capital' => 'Pietermaritzburg', 'website' => 'https://www.kzn.gov.za', 'youtube' => 'https://youtube.com/@KZNProvince'],
        'Limpopo' => ['abbr' => 'LP', 'capital' => 'Polokwane', 'website' => 'https://www.limpopo.gov.za', 'youtube' => 'https://youtube.com/@LimpopoProvince'],
        'Western Cape' => ['abbr' => 'WC', 'capital' => 'Cape Town', 'website' => 'https://www.westerncape.gov.za', 'youtube' => 'https://youtube.com/@WesternCapeGov'],
    ];
@endphp
    ...
    <header class="fixed inset-x-0 top-0 z-50 border-b border-gray-200/80 bg-white/80 backdrop-blur-xl shadow-sm">
        <div class="mx-auto flex max-w-7xl items-center justify-between px-4 sm:px-6 py-3.5">
            ...
            <nav class="hidden md:flex items-center gap-1">
                ...
                <a href="{{ route('gov-sites') }}" class="rounded-lg px-3.5 py-2 text-sm font-medium text-white bg-gradient-to-r from-blue-600 to-blue-700 shadow-sm transition-all">Gov Site</a>
        ...
        <div id="mobile-menu" class="hidden md:hidden border-t border-gray-200/80 bg-white/95 backdrop-blur-xl">
            <div class="px-4 py-3 space-y-1">
                ...
                <a href="{{ route('gov-sites') }}" class="block rounded-lg px-3 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-blue-600 to-blue-700">Gov Site</a>
    ...
    <main class="pt-24 pb-16 min-h-screen">
        <div class="mx-auto max-w-7xl px-4 sm:px-6">
            <div class="animate-fade-up text-center mb-12">
                <span class="inline-block rounded-full border border-blue-200/60 bg-gradient-to-r from-blue-50 to-blue-100/50 px-4 py-1.5 text-xs font-semibold tracking-wider uppercase text-blue-700 shadow-sm">South Africa</span>
                <h1 class="mt-4 text-3xl sm:text-4xl font-bold tracking-tight text-gray-900">Provincial Government Sites</h1>
                <p class="mt-3 text-base text-gray-500 max-w-xl mx-auto">Official websites and social channels for South Africa's provincial governments — part of the <strong class="text-gray-700">Pokhara Research Centre</strong> network.</p>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                @foreach ($provinces as $province)
                    @php
                        $data = $provinceData[$province] ?? ['abbr' => substr($province, 0, 3), 'capital' => '', 'website' => '#', 'youtube' => '#'];
                        $constituencies = Constituency::inProvince($province)->get();
                        $idx++;
                    @endphp
                    <div class="animate-fade-up animate-fade-up-d{{ min($idx, 5) }} group relative rounded-xl border border-gray-200/60 bg-white p-6 transition-all hover:border-blue-200/80 hover:shadow-xl hover:-translate-y-0.5 shadow-sm">
                        ...
                        <div class="flex flex-col items-center text-center">
                            <div class="mb-4 flex h-20 w-20 items-center justify-center rounded-xl bg-gradient-to-br from-blue-50 to-blue-100/80 shadow-sm group-hover:shadow-md group-hover:from-blue-100 group-hover:to-blue-200/80 transition-all">
                                <span class="text-2xl font-extrabold text-blue-600">{{ $data['abbr'] }}</span>
                            </div>

                            <h3 class="text-xl font-bold text-gray-900">{{ $province }}</h3>

                            @if ($data['capital'])
                                <p class="mt-1 text-sm text-gray-500">Capital: {{ $data['capital'] }}</p>
                            @endif

                            <div class="mt-4 w-full rounded-lg bg-gradient-to-br from-gray-50 to-gray-100/50 p-3 border border-gray-100/80">
                                <div class="text-xs text-gray-500">Constituencies</div>
                                <div class="mt-1 flex flex-wrap justify-center gap-1.5">
                                    @foreach ($constituencies as $c)
                                        <span class="inline-block rounded-md bg-white border border-gray-200/60 px-2 py-0.5 text-xs text-gray-600 shadow-sm">{{ $c->name }}</span>
                                    @endforeach
                                </div>
                            </div>
                            ...
                            <div class="mt-5 flex w-full gap-3">
                                ...
                                <a href="{{ $data['youtube'] }}" target="_blank" rel="noopener"
                                   class="flex flex-1 items-center justify-center gap-2 rounded-lg border border-gray-200 bg-white/80 px-4 py-2.5 text-sm font-medium text-gray-700 shadow-sm hover:bg-white hover:border-gray-300 hover:shadow-md transition-all backdrop-blur-sm">
